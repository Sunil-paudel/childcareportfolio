      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-3">
        ...
        <div className="flex md:hidden items-center space-x-1.5 shrink-0">
          ...
          <button
            ...
            className={`p-2 rounded-xl min-w-[44px] min-h-[44px] flex items-center justify-center transition-colors ${mobileMenuOpen
      ...
      <div className="hidden md:block border-t border-stone-200 bg-stone-50/80 overflow-x-auto scrollbar-none">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex space-x-1 py-1.5 min-w-max">
          {CONTEXTS_DATA.map((context, index) => {
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 px-3.5 py-2 rounded-lg text-xs font-semibold tracking-tight transition-all duration-200 whitespace-nowrap ${isActive
              ...
              >
                ...
                <span
                  className={`text-[10px] px-1.5 py-0.2 rounded font-normal ${isActive ? 'bg-stone-800 text-stone-300' : 'bg-stone-200 text-stone-600'
                    }`}
                >
                  Tab {index + 1}
                </span>
          ...
          <button
            ...
            className={`flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold tracking-tight transition-all duration-200 whitespace-nowrap ${activeTab === 'references'
              ? 'bg-stone-900 text-white shadow-xs'
              : 'text-stone-600 hover:text-stone-900 hover:bg-stone-200/70'
              }`}
          >
          ...
          <div className="md:hidden border-t border-stone-200 bg-white animate-in slide-in-from-top-2 duration-200 shadow-xl">
            <div className="px-4 py-3 space-y-1 max-h-[calc(100vh-140px)] overflow-y-auto">
              ...
              {CONTEXTS_DATA.map((context, index) => {
                ...
                return (
                  <button
                    key={context.id}
                    onClick={() => handleSelectTab(context.id)}
                    className={`w-full flex items-center justify-between p-3 rounded-xl text-left transition-all min-h-[50px] ${isActive
                      ? 'bg-stone-900 text-white shadow-xs'
                      : 'text-stone-800 hover:bg-stone-100 active:bg-stone-200/80 border border-stone-100'
                      }`}
                  >
                    <div className="flex items-center space-x-3 min-w-0">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isActive ? 'bg-stone-800 text-amber-400' : 'bg-stone-100 text-stone-600'
                        }`}>
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ml-2 ${isActive ? 'bg-stone-800 text-amber-300' : 'bg-stone-200 text-stone-700'
                      }`}>
                      Tab {index + 1}
                    </span>
                  </button>
              ...
              <div className="pt-2">
                ...
                <button
                  onClick={() => handleSelectTab('references')}
                  className={`w-full flex items-center justify-between p-3 rounded-xl text-left transition-all min-h-[50px] ${activeTab === 'references'
                    ? 'bg-stone-900 text-white shadow-xs'
                    : 'text-stone-800 hover:bg-stone-100 active:bg-stone-200/80 border border-stone-100'
                    }`}
                >
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${activeTab === 'references' ? 'bg-stone-800 text-amber-400' : 'bg-stone-100 text-stone-600'
                      }`}>
